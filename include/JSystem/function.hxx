...
#define __cpp_lib_transparent_operators 201510L
    ...
    template <> struct plus<void> {
        ...
        constexpr auto operator()(_Tp &&__t, _Up &&__u) const
            noexcept(noexcept(forward<_Tp>(__t) + forward<_Up>(__u)))
                -> decltype(forward<_Tp>(__t) + forward<_Up>(__u)) {
            return forward<_Tp>(__t) + forward<_Up>(__u);
        }
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    ...
    // Partial specialization of less for pointers.
    ...
    // Partial specialization of greater_equal for pointers.
    template <typename _Tp>
    ...
    // Partial specialization of less_equal for pointers.
...
#pragma GCC diagnostic pop
    ...
    template <> struct greater<void> {
        ...
        constexpr auto operator()(_Tp &&__t, _Up &&__u) const
            noexcept(noexcept(forward<_Tp>(__t) > forward<_Up>(__u)))
                -> decltype(forward<_Tp>(__t) > forward<_Up>(__u)) {
            return _S_cmp(forward<_Tp>(__t), forward<_Up>(__u), __ptr_cmp<_Tp, _Up>{});
        }
    ...
    private:
        ...
        static constexpr bool _S_cmp(_Tp &&__t, _Up &&__u, true_type) noexcept {
            return greater<const volatile void *>{}(
                static_cast<const volatile void *>(forward<_Tp>(__t)),
                static_cast<const volatile void *>(forward<_Up>(__u)));
        }
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
     ...
     *  };
     *
     *  find_if (v.begin(), v.end(), not1(IntGreaterThanThree()));
     *  \endcode
    ...
    // Partial specialization, avoids confusing errors in e.g. set<const T>.
