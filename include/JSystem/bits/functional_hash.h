...
namespace JSystem {
     ...
     *  @ingroup functors
     *
     *   Hashing functors taking a variable type and returning a @c JSystem::size_t.
     *
     *  @{
     */
...
#undef _Cxx_hashtable_define_trivial_hash
    ...
    template <> struct hash<float> : public __hash_base<size_t, float> {
        size_t operator()(float __val) const noexcept {
            ...
            return __val != 0.0f ? JSystem::_Hash_impl::hash(__val) : 0;
        }
    };
    ...
    template <> struct hash<double> : public __hash_base<size_t, double> {
        size_t operator()(double __val) const noexcept {
            // 0 and -0 both hash to zero.
            return __val != 0.0 ? JSystem::_Hash_impl::hash(__val) : 0;
        }
    };
     ...
     */
    template <typename _Hash> struct __is_fast_hash : public JSystem::true_type {};
